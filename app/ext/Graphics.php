...
class Graphics
{
    ...
    {
        // 404: рендерим статическую страницу из app/page/general/error404.php
        if ($page === 'module_page_404') {
            $error404File = $this->appDir . '/page/general/error404.php';
            if (is_file($error404File)) {
                ob_start();
                include $error404File;
                $content = ob_get_clean();
                return $this->wrapInLayout($content, 'Страница не найдена');
            }
        }
        ...
        // Если нужно вернуть только контент (без layout)
        if (!empty($renderVars['__content_only'])) {
            return $content;
        }

        // Оборачиваем в общий шаблон
        $title = $meta['title'] ?? 'Senior CMS';

        if (!empty($renderVars['__title_override']) && is_string($renderVars['__title_override'])) {
            $title = $renderVars['__title_override'];
        }
    ...
    /**
     * Оборачивает переданный контент в общий layout один раз.
     * Используется, когда на один route нужно отрендерить несколько модулей.
     */
    public function renderLayoutFromContent(string $content, string $title = 'Senior CMS'): string
    {
        return $this->wrapInLayout($content, $title);
    }
    ...
    private function wrapInLayout(string $content, string $title = 'Senior CMS'): string
    {
        ...
        // Main content
        echo '<main class="main"><div class="">';
        echo $content;
        echo '</div></main>';
